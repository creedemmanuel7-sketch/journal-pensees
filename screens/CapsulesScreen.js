...
export default function CapsulesScreen({ navigation }) {
  ...
  const readyNotes = getActiveNotes().filter(
    (n) => n.capsule && new Date(n.capsule) <= new Date(),
  );

  // Tutoriel contextuel des capsules
  const selectTargetRef = useTutorialRef('capsules-select');
  const readyTargetRef = useTutorialRef('capsules-ready');
  const sealedTargetRef = useTutorialRef('capsules-sealed');
  const TUTORIAL_STEPS = [
    ...(readyNotes.length > 0
      ? [
          {
            targetId: 'capsules-ready',
            title: 'Prêtes à ouvrir',
            description:
              'Ces capsules ont atteint leur date : touchez-les pour relire la pensée scellée.',
          },
        ]
      : []),
    {
      targetId: 'capsules-sealed',
      title: 'Capsules scellées',
      description:
        'Elles restent fermées jusqu’à leur date. Touchez une capsule pour la sélectionner.',
    },
    ...(sealedNotes.length > 0
      ? [
          {
            targetId: 'capsules-select',
            title: 'Tout sélectionner',
            description:
              'Sélectionnez toutes les capsules d’un coup, puis utilisez « Desceller » en bas.',
          },
        ]
      : []),
    {
      ...
      description:
        'Dans l’éditeur, ouvrez le menu + puis ⏳ pour sceller une note jusqu’à une date future.',
    },
  ];
  useScreenTutorial('capsules', TUTORIAL_STEPS, { enabled: !isDecoyMode });
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.bg2 }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        {readyNotes.length > 0 && (
          <>
            <Text
              ref={readyTargetRef}
              style={[styles.sectionLabel, { color: accent.teal }]}
            >
            ...
            {readyNotes.map((note) => (
              ...
              >
                <View style={styles.capsuleCardHeader}>
                  ...
                  >
                    <Text style={[styles.readyText, { color: theme.bg }]}>
                      OUVRIR
                    </Text>
                  </View>
                </View>
        ...
        <Text
          ref={sealedTargetRef}
          style={[styles.sectionLabel, { color: theme.text3 }]}
        >
        ...
        {sealedNotes.length === 0 && readyNotes.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            <Text style={[styles.emptyDesc, { color: theme.text3 }]}>
              Scellez une note dans l'éditeur avec ⏳ pour créer votre première
              capsule
            </Text>
          </View>
        ) : (
          sealedNotes.map((note) => (
            ...
            >
              <View style={styles.capsuleCardHeader}>
                ...
                <View style={{ flex: 1 }}>
                  <Text
                    style={[styles.capsuleTitle, { color: theme.text2 }]}
                    numberOfLines={1}
                  >
                    Note scellée
                  </Text>
              ...
              <View
                style={[styles.progressBar, { backgroundColor: theme.bg4 }]}
              >
                <View
                  style={[
                    styles.progressFill,
              ...
              <View style={styles.capsuleDates}>
                ...
                <Text
                  style={[styles.capsuleDateText, { color: accent.primary }]}
                >
      ...
      {selected.length > 0 && (
        ...
        >
          ...
          >
            <Text style={[styles.actionBtnText, { color: accent.primary }]}>
              🔓 Desceller
            </Text>
          </TouchableOpacity>
        </View>
