...
import { computeUsualHour } from '../utils/dates';

const SMART_OPTIONS = [
  {
    key: 'inactivity',
    prefKey: PREF_KEYS.inactivity,
    icon: '🕯️',
    title: "Relance d'inactivité",
    desc: 'Un rappel doux après quelques jours sans écrire.',
  },
  {
    key: 'streak',
    prefKey: PREF_KEYS.streak,
    icon: '🔥',
    title: 'Protection de série',
    desc: "Le soir, si tu n'as pas encore écrit, garde ta série en vie.",
  },
  {
    key: 'capsule',
    prefKey: PREF_KEYS.capsule,
    icon: '⏳',
    title: 'Capsule prête',
    desc: 'Quand une capsule temporelle arrive à échéance.',
  },
  {
    key: 'onthisday',
    prefKey: PREF_KEYS.onthisday,
    icon: '📖',
    title: 'Il y a un an',
    desc: "Quand un souvenir d'il y a environ un an t'attend.",
  },
];
...
export default function NotificationsScreen({ navigation }) {
  ...
  // Heure d'écriture habituelle déduite des notes réelles (jamais en leurre).
  const usualHour = isDecoy ? null : computeUsualHour(notes);
  ...
  const handleSmartToggle = async (option, val) => {
    if (val) {
    ...
    setSmartPrefs((prev) => ({ ...prev, [option.key]: val }));
    await setSmartPref(option.prefKey, val);
    // Replanifie uniquement pour l'utilisateur réel ; rien en mode leurre.
    await replanAllSmart(notes, { isDecoy });
  };

  const applyUsualHour = async () => {
    if (usualHour == null) return;
    await handleSave(usualHour, 0);
  };
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        >
          <View style={styles.mainCardLeft}>
            ...
            <Text style={[styles.mainCardTitle, { color: theme.text }]}>
              Rappels d'écriture
            </Text>
        ...
        <View style={styles.quickGrid}>
          {QUICK_TIMES.map((qt) => (
            ...
            >
              ...
              <Text style={[styles.quickSublabel, { color: theme.text3 }]}>
                {qt.sublabel}
              </Text>
        ...
        >
          <Text style={[styles.customLabel, { color: theme.text3 }]}>
            CHOISIR L'HEURE
          </Text>
          ...
          >
            ...
            >
              {saving
                ? 'Programmation...'
                : `Programmer à ${formatTime(hour, minute)}`}
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <Text style={[styles.smartIntro, { color: theme.text3 }]}>
          Des rappels adaptatifs, sans jamais révéler le contenu de tes notes.
        </Text>
        {SMART_OPTIONS.map((option) => (
          ...
          >
            <Text style={styles.smartIcon}>{option.icon}</Text>
            <View style={styles.smartTextWrap}>
              <Text style={[styles.smartTitle, { color: theme.text }]}>
                {option.title}
              </Text>
              <Text style={[styles.smartDesc, { color: theme.text3 }]}>
                {option.desc}
              </Text>
            </View>
            <Switch
              value={smartPrefs[option.key]}
              onValueChange={(val) => handleSmartToggle(option, val)}
              trackColor={{ false: theme.bg5, true: accent.primary }}
              thumbColor="#fff"
            />
          </View>
        ...
        {/* Horaire intelligent (suggestion) */}
        {usualHour != null && (
          <>
            <Text style={[styles.sectionLabel, { color: theme.text3 }]}>
              HORAIRE INTELLIGENT
            ...
            >
              ...
              <View style={styles.smartTextWrap}>
                <Text style={[styles.smartTitle, { color: theme.text }]}>
                  Heure habituelle : {formatTime(usualHour, 0)}
                </Text>
                <Text style={[styles.smartDesc, { color: theme.text3 }]}>
                  Tu écris souvent vers cette heure. Programmer le rappel à ce
                  moment-là ?
                </Text>
              </View>
              <TouchableOpacity
                style={[
                  styles.smartApplyBtn,
                  { backgroundColor: accent.primary },
                ]}
                onPress={applyUsualHour}
              >
                <Text style={[styles.smartApplyText, { color: theme.bg }]}>
                  Utiliser
                </Text>
              </TouchableOpacity>
            </View>
        ...
        {enabled && (
          <>
            ...
            >
              <Text style={{ fontSize: 16 }}>🔕</Text>
              <Text style={[styles.cancelBtnText, { color: '#e55' }]}>
...
const styles = StyleSheet.create({
  ...
  smartTitle: { fontSize: 14, fontWeight: '500', marginBottom: 3 },
  smartDesc: { fontSize: 12, lineHeight: 17 },
  smartApplyBtn: {
    paddingHorizontal: 14,
    paddingVertical: 8,
    borderRadius: 10,
  },
  smartApplyText: { fontSize: 13, fontWeight: '500' },
});
