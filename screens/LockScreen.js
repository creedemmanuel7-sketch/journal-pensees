...
function AnimatedLines({ accent }) {
  ...
  const r1 = anim1.interpolate({
    inputRange: [0, 1],
    outputRange: ['-15deg', '-10deg'],
  });
  const r2 = anim2.interpolate({
    inputRange: [0, 1],
    outputRange: ['25deg', '30deg'],
  });
  const r3 = anim3.interpolate({
    inputRange: [0, 1],
  ...
  return (
    <View style={StyleSheet.absoluteFill} pointerEvents="none">
      <Animated.View
        style={[lineStyle(r1, t1), { top: height * 0.22, backgroundColor: c }]}
      />
      <Animated.View
        style={[
          lineStyle(r1, t1),
          { top: height * 0.22 + 2, backgroundColor: c2 },
        ]}
      />
      <Animated.View
        style={[lineStyle(r2, t2), { top: height * 0.38, backgroundColor: c }]}
      />
      <Animated.View
        style={[
          lineStyle(r2, t2),
          { top: height * 0.38 + 2, backgroundColor: c2 },
        ]}
      />
      <Animated.View
        style={[lineStyle(r3, t3), { top: height * 0.55, backgroundColor: c }]}
      />
      <Animated.View
        style={[
          lineStyle(r3, t3),
          { top: height * 0.55 + 2, backgroundColor: c2 },
        ]}
      />
      <Animated.View
        style={[lineStyle(r1, t2), { top: height * 0.7, backgroundColor: c2 }]}
      />
      <Animated.View
        style={[lineStyle(r2, t1), { top: height * 0.82, backgroundColor: c }]}
      />
    </View>
  );
}
...
function LockIcon({ accent }) {
  return (
    <Svg width="70" height="80" viewBox="0 0 140 160">
      ...
      <Path
        d="M35,65 L35,35 Q35,5 70,5 Q105,5 105,35 L105,65"
        fill="none"
        stroke="url(#lg)"
        strokeWidth="18"
        strokeLinecap="round"
...
export default function LockScreen({ navigation }) {
  ...
  const { deactivateDecoyMode, encryptionReady, unlockWithStoredKey } =
    useNotes();
  ...
  }, [authReady, pinConfigured, biometricAvailable, biometricEnabled]);
  ...
  const handleBiometric = async () => {
    ...
    if (!encryptionReady) {
      const ok = await unlockWithStoredKey();
      if (!ok) {
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.bg }]}>
      ...
      >
        ...
        <View style={styles.protectedRow}>
          ...
          <Text style={[styles.protectedText, { color: theme.text3 }]}>
            PROTÉGÉ
          </Text>
        </View>
      </Animated.View>
      ...
      <TouchableOpacity onPress={() => navigation.navigate('Pin')}>
        <Text style={[styles.pinLink, { color: theme.text4 }]}>
          SAISIR LE CODE PIN
        </Text>
      </TouchableOpacity>
